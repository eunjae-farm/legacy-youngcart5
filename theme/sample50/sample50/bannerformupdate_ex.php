...
@mkdir(G5_DATA_PATH . "/banner/mobile", G5_DIR_PERMISSION);
@chmod(G5_DATA_PATH . "/banner/mobile", G5_DIR_PERMISSION);


$bn_bimg      = isset($_FILES['bn_bimg']['tmp_name']) ? $_FILES['bn_bimg']['tmp_name'] : '';
$bn_bimg_name = isset($_FILES['bn_bimg']['name']) ? $_FILES['bn_bimg']['name'] : '';

$bn_bimg_mobile      = isset($_FILES['bn_bimg_mobile']['tmp_name']) ? $_FILES['bn_bimg_mobile']['tmp_name'] : '';
$bn_bimg_mobile_name = isset($_FILES['bn_bimg_mobile']['name']) ? $_FILES['bn_bimg_mobile']['name'] : '';
...
$bn_bimg_mobile_del = (isset($_POST['bn_bimg_mobile_del']) && $_POST['bn_bimg_mobile_del']) ? preg_replace('/[^0-9]/', '', $_POST['bn_id']) : 0;

$bn_url = isset($_POST['bn_url']) ? strip_tags(clean_xss_attributes($_POST['bn_url'])) : '';
$bn_alt = isset($_POST['bn_alt']) ? strip_tags(clean_xss_attributes($_POST['bn_alt'])) : '';
...
//@tlog 필드추가
$bn_bg = isset($_POST['bn_bg']) ? strip_tags(clean_xss_attributes($_POST['bn_bg'])) : '';
$bn_img_title = isset($_POST['bn_img_title']) ? strip_tags(clean_xss_attributes($_POST['bn_img_title'])) : '';
$bn_img_subtitle = isset($_POST['bn_img_subtitle']) ? strip_tags(clean_xss_attributes($_POST['bn_img_subtitle'])) : '';

$bn_json = sql_real_escape_string(json_encode(array('bg' => $bn_bg, 'img_title' => $bn_img_title, 'img_subtitle' => $bn_img_subtitle), JSON_UNESCAPED_UNICODE));


if ($bn_bimg_del)  @unlink(G5_DATA_PATH . "/banner/$bn_id");
if ($bn_bimg_mobile_del)  @unlink(G5_DATA_PATH . "/banner/mobile/{$bn_id}");
...
if ($bn_bimg_mobile || $bn_bimg_mobile_name) {

    if (!preg_match('/\.(gif|jpe?g|bmp|png)$/i', $bn_bimg_mobile_name)) {
        alert("이미지 파일만 업로드 할수 있습니다.");
    }

    $timg = @getimagesize($bn_bimg_mobile);
    if ($timg['2'] < 1 || $timg['2'] > 16) {
        alert("이미지 파일만 업로드 할수 있습니다.");
    }
}
...
if ($w == "") {
    ...
    //@tlog 반응형 이미지
    @unlink(G5_DATA_PATH . "/banner/mobile/$bn_id");
...
if ($w == "" || $w == "u") {
    ...
    if ($_FILES['bn_bimg_mobile']['name']) upload_file($_FILES['bn_bimg_mobile']['tmp_name'], $bn_id, G5_DATA_PATH . "/banner/mobile");
