...
$bn_position = (isset($_GET['bn_position']) && in_array($_GET['bn_position'], array('메인', '왼쪽', '상단'))) ? $_GET['bn_position'] : '';
...
<div class="local_ov01 local_ov">
    ...
    <select name="bn_position" id="bn_position">
        ...
        <option value="상단"<?php echo get_selected($bn_position, '상단', true); ?>>상단</option>
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $bimg_mobile = G5_DATA_PATH.'/banner/mobile/'.$row['bn_id'];
        if(file_exists($bimg_mobile)) {
            ...
            $bn_img_mobile .= '<div style="margin-top:10px;">반응형일때 모바일<br><img src="'.G5_DATA_URL.'/banner/mobile/'.$row['bn_id'].'?'.preg_replace('/[^0-9]/i', '', $row['bn_time']).'" width="'.$width.'" alt="'.get_text($row['bn_alt']).'"></div>';
        }
    ...
    <tr class="<?php echo $bg; ?>">
        <td headers="th_img" colspan="7" class="td_img_view sbn_img">
            <div class="sbn_image"><?php echo $bn_img; ?><?php echo $bn_img_mobile; ?></div>
